...
for i, state in enumerate(solution_path):
    ...
    if i > 0:
        ...
        for row in range(3):
            for col in range(3):
                if state[row][col] != prev_state[row][col]:
                    if state[row][col] == 0:
                        print(f"Mueve {prev_state[row][col]} hacia arriba:")
                    elif state[row][col] == prev_state[row][col]:
                        print(f"Mantiene {state[row][col]} en su lugar:")
                    else:
                        print(f"Mueve {state[row][col]} hacia abajo:")
        print()
